...
function getDevices($conn) {
    $query = "SELECT * FROM biometric_devices ORDER BY location, device_name";
...
function toggleDevice($conn, $data) {
    ...
    $stmt = $conn->prepare("UPDATE biometric_devices SET is_active = ?, updated_at = NOW() WHERE id = ?");
...
function getSyncStatus($conn) {
    $query = "SELECT ds.*, bd.device_name, bd.device_type 
              FROM device_sync_status ds 
              LEFT JOIN biometric_devices bd ON ds.device_id = bd.id 
              ORDER BY ds.campus_name, ds.last_sync DESC";
    $result = $conn->query($query);
...
function createDevice($conn, $data) {
    ...
    try {
        ...
        // Create the device
        $sql = "INSERT INTO biometric_devices (device_name, device_type, location, is_active, created_at, updated_at) 
                VALUES ('$device_name', '$device_type', '$location', $is_active, NOW(), NOW())";
        ...
        $sync_sql = "INSERT INTO device_sync_status (device_id, campus_name, sync_status, last_sync, created_at, updated_at) 
                     VALUES ($device_id, '$location', 'pending', NOW(), NOW(), NOW())";
