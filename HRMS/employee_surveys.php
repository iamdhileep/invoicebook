...
$query = "SELECT 
    ...
    start_date,
    end_date,
    response_count as responses,
    target_employees as total_employees,
    completion_rate,
    is_anonymous,
    created_by
...
if ($result) {
    ...
    echo "Error fetching surveys: " . mysqli_error($conn);
}

// Database-driven survey templates
$survey_templates = [];
$query = "SHOW COLUMNS FROM employee_surveys LIKE 'survey_type'";
$result = mysqli_query($conn, $query);
if ($result && $row = mysqli_fetch_assoc($result)) {
    ...
    if ($matches) {
        ...
        $survey_templates = $types;
    }
}

// Fallback templates if database query fails
if (empty($survey_templates)) {
    $survey_templates = [
        'Employee Engagement',
        'Exit Interview',
        'Training Feedback',
