...
// Total Hours Today (from time_tracking table or attendance work_duration)
$total_hours_today_query = "SELECT SUM(work_duration) as total_hours FROM attendance WHERE date = '$today'";
...
$active_employees_query = "SELECT COUNT(DISTINCT employee_id) as count FROM attendance WHERE date = '$today' AND status != 'absent'";
...
$avg_hours_query = "SELECT AVG(work_duration) as avg_hours FROM attendance WHERE date = '$today' AND work_duration > 0";
