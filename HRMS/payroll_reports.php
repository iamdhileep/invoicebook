...
// Database-driven payroll report data
$monthly_reports = [];
$query = "SELECT 
    CONCAT(report_month, ' ', report_year) as month,
    employee_count as employees,
    total_gross_salary as total_salary,
    total_deductions as deductions,
    total_net_salary as net_payroll,
...
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $monthly_reports[] = [
            ...
            'net_payroll' => $row['net_payroll'],
            'status' => $row['status']
        ];
    }
} else {
    echo "Error fetching payroll reports: " . mysqli_error($conn);
}

$current_page = 'payroll_reports';
