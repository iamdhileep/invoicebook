...
$leaveTypes = [];
try {
    $result = $conn->query("SELECT * FROM hr_leave_types WHERE is_active = 1 ORDER BY name");
    while ($row = $result->fetch_assoc()) {
        $leaveTypes[] = $row;
    }
} catch (Exception $e) {
...
if ($currentUserRole === 'hr' || $currentUserRole === 'admin') {
    try {
        $result = $conn->query("SELECT id, employee_id, first_name, last_name, department_id FROM hr_employees WHERE is_active = 1 ORDER BY first_name, last_name");
        while ($row = $result->fetch_assoc()) {
            $employees[] = $row;
        }
    } catch (Exception $e) {
