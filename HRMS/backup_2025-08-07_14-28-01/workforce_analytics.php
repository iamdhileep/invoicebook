...
// Fetch key metrics
$total_workforce_query = "SELECT COUNT(*) as total FROM hr_employees WHERE status = 'active'";
$total_workforce = $mysqli->query($total_workforce_query)->fetch_assoc()['total'];

$new_hires_query = "SELECT COUNT(*) as new_hires FROM hr_employees WHERE hire_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND status = 'active'";
$new_hires = $mysqli->query($new_hires_query)->fetch_assoc()['new_hires'];

$avg_age_query = "SELECT AVG(DATEDIFF(CURDATE(), date_of_birth) / 365) as avg_age FROM hr_employees WHERE status = 'active' AND date_of_birth IS NOT NULL";
$avg_age_result = $mysqli->query($avg_age_query);
$avg_age = $avg_age_result ? round($avg_age_result->fetch_assoc()['avg_age'], 1) : 0;

$female_count_query = "SELECT COUNT(*) as female_count FROM hr_employees WHERE status = 'active'"; // Note: Assuming gender column exists, otherwise use a default calculation
                ...
                <div class="card text-center metric-card bg-info text-white">
                    <div class="card-body">
                        ...
                        <h3>
                            <?php 
                            $today = date('Y-m-d');
                            $attendance_query = "SELECT COUNT(DISTINCT employee_id) as present_count FROM hr_attendance WHERE date = '$today' AND status = 'present'";
            ...
            <div class="col-md-3">
                <div class="card text-center metric-card bg-warning text-white">
                    <div class="card-body">
                        ...
                        <h3>
                            <?php 
                            $performance_query = "SELECT AVG(rating) as avg_rating FROM hr_performance_reviews WHERE rating IS NOT NULL";
