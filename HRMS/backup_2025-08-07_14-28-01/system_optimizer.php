...
$queries = [
    'total_employees' => "SELECT COUNT(*) as count FROM hr_employees",
    'active_employees' => "SELECT COUNT(*) as count FROM hr_employees WHERE status = 'active'",
    'employees_with_missing_data' => "SELECT COUNT(*) as count FROM hr_employees WHERE email IS NULL OR phone IS NULL OR joining_date IS NULL",
    'recent_hires' => "SELECT COUNT(*) as count FROM hr_employees WHERE joining_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAYS)",
    'duplicate_emails' => "SELECT COUNT(*) - COUNT(DISTINCT email) as count FROM hr_employees WHERE email IS NOT NULL",
    'performance_records' => "SELECT COUNT(*) as count FROM employee_performance",
    'attendance_coverage' => "SELECT COUNT(DISTINCT employee_id) as count FROM hr_attendance WHERE attendance_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAYS)",
...
$integrity_checks = [
    'Foreign Key Consistency' => [
        "SELECT COUNT(*) as issues FROM hr_employees e LEFT JOIN hr_departments d ON e.department_id = d.department_id WHERE e.department_id IS NOT NULL AND d.department_id IS NULL",
        "Employees with invalid department references"
    ],
    'Attendance Data Validity' => [
        "SELECT COUNT(*) as issues FROM hr_attendance WHERE attendance_date > CURDATE() OR attendance_date < '2020-01-01'",
