...
class HRMSRoleManager {
    ...
    public function getAccessibleEmployees() {
        switch ($this->current_user_role) {
            case 'Admin':
                return "SELECT * FROM hr_employees"; // All employees
                
            case 'Manager':
                // Get employees in same department or reporting to this manager
                return "SELECT e.* FROM hr_employees e 
                        LEFT JOIN hr_employees mgr ON mgr.id = " . $this->current_user_id . "
                        WHERE e.department_id = mgr.department_id 
                        OR e.manager_id = " . $this->current_user_id;
                
            case 'Employee':
                // Only own record
                return "SELECT * FROM hr_employees WHERE user_id = " . $this->current_user_id;
                
            default:
                return "SELECT * FROM hr_employees WHERE 1=0"; // No access
        }
    }
