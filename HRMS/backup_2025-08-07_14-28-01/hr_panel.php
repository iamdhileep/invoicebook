...
try {
    ...
    $result = $conn->query("SELECT COUNT(*) as total FROM hr_employees");
    if ($result && $row = $result->fetch_assoc()) {
        $hrStats['total_employees'] = (int)$row['total'];
    }

    // Active employees count
    $result = $conn->query("SELECT COUNT(*) as active FROM hr_employees WHERE is_active = 1");
    if ($result && $row = $result->fetch_assoc()) {
    ...
    // Today's attendance
    $today = date('Y-m-d');
    $result = $conn->query("SELECT COUNT(*) as present FROM hr_attendance WHERE attendance_date = '$today' AND status IN ('present', 'late')");
    if ($result && $row = $result->fetch_assoc()) {
    ...
    $result = $conn->query("SELECT COUNT(*) as new_hires FROM hr_employees WHERE DATE_FORMAT(created_at, '%Y-%m') = '$currentMonth'");
    if ($result && $row = $result->fetch_assoc()) {
