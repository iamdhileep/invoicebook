...
$query = "
    ...
    FROM hr_performance_reviews pr
    JOIN hr_employees e ON pr.employee_id = e.employee_id
    JOIN hr_employees r ON pr.reviewer_id = r.employee_id
...
$query = "
    ...
    JOIN hr_employees e ON eg.employee_id = e.employee_id
    JOIN hr_employees a ON eg.assigned_by = a.employee_id
...
$employees = [];
$query = "SELECT employee_id, name, employee_code, position FROM hr_employees WHERE status = 'active' ORDER BY name";
$result = mysqli_query($conn, $query);
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $employees[] = $row;
