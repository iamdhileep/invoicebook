...
$totalEmployees = 0;
$activeEmployees = 0;
$result = mysqli_query($conn, "SELECT COUNT(*) as total FROM hr_employees");
if ($result) {
    $row = mysqli_fetch_assoc($result);
    $totalEmployees = $row['total'];
}

$result = mysqli_query($conn, "SELECT COUNT(*) as active FROM hr_employees WHERE status = 'active'");
if ($result) {
    $row = mysqli_fetch_assoc($result);
...
// Average Salary
$avgSalary = 0;
$result = mysqli_query($conn, "SELECT AVG(monthly_salary) as avg_salary FROM hr_employees WHERE status = 'active' AND monthly_salary > 0");
if ($result) {
    $row = mysqli_fetch_assoc($result);
...
$newJoiners = 0;
$result = mysqli_query($conn, "SELECT COUNT(*) as new_joiners FROM hr_employees WHERE DATE_FORMAT(joining_date, '%Y-%m') = '$currentMonth'");
if ($result) {
    $row = mysqli_fetch_assoc($result);
