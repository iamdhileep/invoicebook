...
$query = "
    ...
    LEFT JOIN hr_employees hr ON op.assigned_hr = hr.employee_id
    LEFT JOIN hr_employees mgr ON op.assigned_manager = mgr.employee_id
    LEFT JOIN hr_employees buddy ON op.buddy_employee_id = buddy.employee_id
...
$hrEmployees = [];
$managerEmployees = [];
$query = "SELECT employee_id, name FROM hr_employees WHERE status = 'active' ORDER BY name";
$result = mysqli_query($conn, $query);
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $hrEmployees[] = $row;
