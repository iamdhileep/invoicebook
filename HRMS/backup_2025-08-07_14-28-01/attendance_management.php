    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'mark_attendance':
                ...
                $query = "INSERT INTO hr_attendance (employee_id, date, clock_in_time, clock_out_time, status, total_hours, notes)
                         VALUES ($employee_id, '$attendance_date', " . 
                         ($check_in_time ? "'$check_in_time'" : 'NULL') . ", " . 
                         ($check_out_time ? "'$check_out_time'" : 'NULL') . ", '$status', $working_hours, '$notes')
                         ON DUPLICATE KEY UPDATE 
                         clock_in_time = " . ($check_in_time ? "'$check_in_time'" : 'clock_in_time') . ",
                         clock_out_time = " . ($check_out_time ? "'$check_out_time'" : 'clock_out_time') . ",
                         status = '$status',
            ...
            case 'bulk_mark_attendance':
                ...
                $employees = mysqli_query($conn, "SELECT employee_id FROM hr_employees WHERE status = 'active'");
                $success_count = 0;
                
                while ($employee = mysqli_fetch_assoc($employees)) {
                    $query = "INSERT INTO hr_attendance (employee_id, attendance_date, status)
...
$statsQuery = "
    ...
    FROM hr_employees e
    LEFT JOIN hr_attendance a ON e.employee_id = a.employee_id AND a.attendance_date = '$today'
...
$query = "
    SELECT e.employee_id, e.name, e.employee_code, e.position, d.department_name,
           a.clock_in_time, a.clock_out_time, a.status, a.total_hours, a.notes
    FROM hr_employees e
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $trendQuery = "
        SELECT 
            ...
            SUM(CASE WHEN a.status = 'Absent' OR a.id IS NULL THEN 1 ELSE 0 END) as absent
        FROM hr_employees e
        LEFT JOIN hr_attendance a ON e.employee_id = a.employee_id AND a.attendance_date = '$date'
        WHERE e.status = 'active'
    ";
...
$employees = [];
$result = mysqli_query($conn, "SELECT employee_id, name, employee_code FROM hr_employees WHERE status = 'active' ORDER BY name");
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $employees[] = $row;
    }
...
// No duplicate includes needed - they're already at the top
?>
                                ...
                                <tbody>
                                    <?php foreach ($todayAttendance as $attendance): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?= $attendance['total_hours'] ? number_format($attendance['total_hours'], 1) . 'h' : '<span class="text-muted">-</span>' ?>
                                            </td>
                                            <td>
                                                <?php
