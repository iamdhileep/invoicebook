...
class GDPRComplianceManager {
    ...
    public function exportUserData($user_id, $format = 'json') {
        ...
        // Get employee data
        $query = "SELECT * FROM hr_employees WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $query);
        ...
        // Get attendance data
        $query = "SELECT * FROM hr_attendance WHERE employee_id = ? ORDER BY date DESC LIMIT 100";
        $stmt = mysqli_prepare($this->conn, $query);
        mysqli_stmt_bind_param($stmt, "i", $user_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        ...
        // Get payroll data
        $query = "SELECT * FROM hr_payroll_calculations WHERE employee_id = ? ORDER BY pay_period DESC LIMIT 12";
        $stmt = mysqli_prepare($this->conn, $query);
        mysqli_stmt_bind_param($stmt, "i", $user_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
