...
if (!HRMSHelper::isLoggedIn()) {
    header('Location: ../hrms_portal.php?redirect=HRMS/control_center.php');
    exit;
}
...
try {
    ...
    $result = HRMSHelper::safeQuery("SELECT COUNT(*) as total FROM hr_employees WHERE is_active = 1");
    ...
    $today = date('Y-m-d');
    $result = HRMSHelper::safeQuery("SELECT COUNT(*) as total FROM hr_attendance WHERE attendance_date = '$today'");
    if ($result && $row = $result->fetch_assoc()) {
        $systemStats['today_attendance'] = (int)$row['total'];
    }

    $result = HRMSHelper::safeQuery("SELECT COUNT(*) as total FROM hr_leave_applications WHERE status = 'pending'");
    if ($result && $row = $result->fetch_assoc()) {
