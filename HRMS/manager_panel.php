...
if (!HRMSHelper::isLoggedIn()) {
    header('Location: ../hrms_portal.php?redirect=HRMS/manager_panel.php');
    exit;
}

if (!HRMSHelper::hasPermission('all') && !HRMSHelper::hasPermission('team_view')) {
...
try {
    ...
    $result = HRMSHelper::safeQuery("SELECT * FROM hr_employees WHERE user_id = '$currentUserId' AND is_active = 1 LIMIT 1");
    if ($result && $result->num_rows > 0) {
        ...
        $result = HRMSHelper::safeQuery("SELECT * FROM hr_employees WHERE is_active = 1 ORDER BY id LIMIT 1");
        if ($result && $result->num_rows > 0) {
            $manager = $result->fetch_assoc();
            $managerId = $manager['id'];
            $managerDepartmentId = $manager['department_id'];
...
try {
    ...
    $result = HRMSHelper::safeQuery("SELECT COUNT(*) as total FROM hr_employees e $teamCondition");
